...
<body>
    <h2>File Upload</h2>
    <?php
    // --- RECONNECT INTEGRATION START ---
    require_once("ReConnect/settings/core.php"); 
    // Ensure user is logged in, otherwise default to a 'guest' folder or redirect
    $user_id = is_logged_in() ? get_user_id() : 'guest';
    // --- RECONNECT INTEGRATION END ---
    ...
    // --- UPDATED UPLOAD DIRECTORY LOGIC ---
    // Organizes files by User ID so they appear in "My Media Library"
    $upload_dir = "uploads/u{$user_id}/misc/"; 
        ...
        if ($file["error"] !== UPLOAD_ERR_OK) {
            ...
            if (empty($filename)) {
                ...
                if ($file["size"] > $max_file_size) {
                    ...
                    // Check if file already exists (timestamp makes this unlikely but good practice)
                    if (file_exists($target_file)) {
                        ...
                        if (move_uploaded_file($file["tmp_name"], $target_file)) {
                            ...
                            // Optionally: Redirect back to ReConnect Media Library
                            // header("Location: ReConnect/admin/media_library.php");
                        } else {
    ...
    <div class="file-list">
        <h3>Uploaded Files (Your Folder):</h3>
        <?php
        if (is_dir($upload_dir)) {
